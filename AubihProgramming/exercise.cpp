...
//  AubihProgramming
//
//  Created by Dejan Misic on 13/11/2018.
...
void Exercise::TaskSeven(){
    ...
    while (word.empty() || (!Helper::IsStringAlphaNumeric(word)) || (Helper::ContainWhitespace(word)) || (word.size() < 1 || (word.size() == 1)))
    {
        if (Helper::CaseInsensitiveStringCompare(word,"exit")){
            break;
        }
        std::cin.clear(); // clear input buffer to restore cin to a usable state
...
void Exercise::TaskEight(){
    ...
    try{
        ...
        if (sentence.size() > 0){
            converted = Helper::StringToUpper(sentence);
...
void Exercise::TaskNine(){
    ...
    try{
        ...
        std::cout<<std::endl;
        if (sentence.size() > 1){
            sentenceHolder=Helper::SplitSentence((char*)sentence.c_str());
            sentence.clear();
...
void Exercise::TaskTen(){
    ...
    try{
        ...
        if (sentence.size() > 1){
            ...
            {
                words += Helper::ReverseWord(sentenceHolder[i]) + " ";
            }
